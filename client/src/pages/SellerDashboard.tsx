...
function SellerVideoSearchPicker({ productName, value, onChange }: { productName: string; value: string; onChange: (v: string) => void }) {
  ...
  const handleAction = () => {
    ...
    if (!trimmed) return;
    if (isUrl) {
      const id = parseYouTubeIdSeller(trimmed);
    ...
    <div>
      {value ? (
        <div className="flex items-center gap-3 bg-white rounded-lg p-2 border border-red-200">
          ...
          <button type="button" onClick={() => { onChange(""); setSelectedTitle(""); setSelectedThumb(""); setInput(productName); setInputError(""); setOpen(false); }}
            className="text-gray-400 hover:text-red-500 shrink-0"><X size={16} /></button>
        </div>
      ) : (
        <div className="space-y-2">
          <p className="text-xs text-gray-500">Введите название для поиска <strong>или вставьте ссылку</strong> на YouTube</p>
          <div className="flex gap-2">
            <input value={input} onChange={e => { setInput(e.target.value); setInputError(""); setOpen(false); }}
            ...
            <button type="button" onClick={handleAction}
              className="flex items-center gap-1 bg-red-600 text-white px-3 py-2 rounded-lg text-xs font-semibold hover:bg-red-700 shrink-0">
              {isUrl ? <>✔ Применить</> : <><Search size={13} /> Найти</>}
            </button>
          </div>
