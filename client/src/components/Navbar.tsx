                    ...
                    {results.map((product, idx) => (
                      <button key={product.id} type="button"
                        ...
                        <div className="w-10 h-10 rounded bg-gray-100 flex items-center justify-center shrink-0 overflow-hidden">
                          {product.imageUrl ? <img src={`/api/img?url=${encodeURIComponent(product.imageUrl)}&w=80&q=75`} alt="" className="w-full h-full object-contain p-1" loading="lazy" /> : <span className="text-xl">📦</span>}
                        </div>
