          ...
          <div className="flex-1 relative">
            ...
            {showDropdown && hasQuery && (
              <div ref={dropdownRef} className="absolute top-full left-0 right-0 mt-1 bg-white rounded-lg shadow-2xl border border-gray-100 overflow-hidden z-50" style={{ maxHeight: "480px", overflowY: "auto" }}>
                ...
                {!isSearching && results.length === 0 && (
                  <div className="px-4 py-3 text-sm text-gray-500 border-b border-gray-100">{t.nav_not_found_on_site} «{searchQuery}»</div>
                )}
                <div className="bg-gray-50">
                  <div className="px-4 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wide">{t.nav_search_internet}</div>
                  <a href={`https://www.google.com/search?q=${encodeURIComponent(searchQuery + " kattachegirma.uz")}`} target="_blank" rel="noopener noreferrer"
                    className={`flex items-center gap-3 px-4 py-2.5 hover:bg-gray-100 transition-colors ${selectedIndex === results.length ? "bg-gray-100" : ""}`} onClick={() => setShowDropdown(false)}>
                    <svg viewBox="0 0 24 24" className="w-5 h-5 flex-shrink-0"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
                    <span className="text-sm text-gray-700 flex-1">{t.nav_search_in_google}: «<strong>{searchQuery}</strong>»</span>
                    <ExternalLink size={12} className="text-gray-400" />
                  </a>
                  <a href={`https://yandex.ru/search/?text=${encodeURIComponent(searchQuery + " kattachegirma.uz")}`} target="_blank" rel="noopener noreferrer"
                    className={`flex items-center gap-3 px-4 py-2.5 hover:bg-gray-100 transition-colors ${selectedIndex === results.length + 1 ? "bg-gray-100" : ""}`} onClick={() => setShowDropdown(false)}>
                    <svg viewBox="0 0 24 24" className="w-5 h-5 flex-shrink-0"><path fill="#FC3F1D" d="M2.04 12c0-5.52 4.44-9.96 9.96-9.96S21.96 6.48 21.96 12 17.52 21.96 12 21.96 2.04 17.52 2.04 12z"/><path fill="#fff" d="M13.32 7.2h-.78c-1.44 0-2.22.72-2.22 1.86 0 1.26.54 1.86 1.68 2.64l.96.66-2.7 4.02H8.82l2.52-3.72c-1.44-.96-2.22-1.98-2.22-3.54 0-1.98 1.38-3.3 3.66-3.3h2.82v10.56h-2.28V7.2z"/></svg>
                    <span className="text-sm text-gray-700 flex-1">{t.nav_search_in_yandex}: «<strong>{searchQuery}</strong>»</span>
                    <ExternalLink size={12} className="text-gray-400" />
                  </a>
                </div>
              </div>
