        ...
        <div ref={containerRef} className="flex items-center gap-1.5 py-2 relative">
          {visibleCats.map((cat) => (
            <Link
              ...
              className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-[11px] font-medium whitespace-nowrap transition-all duration-100 border active:scale-[0.97] active:brightness-95 touch-manipulation select-none ${
                isActive(cat)
                ...
                <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-2xl z-50 min-w-[180px] py-1 overflow-hidden">
                  {hiddenCats.map((cat) => (
                    <Link
                      ...
                      href={`/category/${cat.slug}`}
                      onClick={() => setShowMore(false)}
                      className={`flex items-center gap-2.5 px-4 py-2 hover:bg-red-50 hover:text-red-700 transition-all duration-100 text-[12px] active:scale-[0.97] active:brightness-95 touch-manipulation select-none ${
                        isActive(cat)
